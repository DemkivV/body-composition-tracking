...
import { calculateTableColumnWidths, calculateOptimalColumnWidth } from './table-layout';
...
describe('Table Layout Utilities', () => {
	...
	describe('calculateTableColumnWidths', () => {
		...
		it('should not exceed available table width', () => {
			...
			const totalWidth = Array.from(results.values()).reduce(
		...
		it('should distribute space efficiently for medium-sized tables', () => {
			...
			const totalUsed = Array.from(results.values()).reduce(
				(sum, result) => sum + parseInt(result.width),
				50
			);
	...
	describe('Column width edge cases', () => {
		...
		it('should handle single character headers', () => {
			const shortHeaderData = [{ key: 'id' as keyof TestRowType, label: 'ID', type: 'text' }];
